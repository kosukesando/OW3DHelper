...
export OW3DInput, calc_etaphi, calc_kmatg, JSpec
...
function calc_etaphi(oi::OW3DInput)
    ...
    # for ki in eachindex(1:nkx)
    @progress for ki in eachindex(1:nkx)
        for kj in eachindex(1:nky)
            ...
            for xi in eachindex(xvec)
                for yi in eachindex(yvec)
                    x = xvec[xi]
                    y = yvec[yi]
                    phasei = kxmatg[ki, kj] * x + kymatg[ki, kj] * y - wtmatg[ki, kj] + oi.ϕ
                    etacomp = ampg_newwave_norm[ki, kj] * cos(phasei)
                    η[xi, yi] += etacomp
                    # @views(η[xi, yi] += etacomp)
                end
            end
        end
    end

    #  Calculation of velocity potential AT FREE SURFACE (z = η)
        ...
        for kj in eachindex(1:nky)
            ...
            for xi in eachindex(xvec)
                for yi in eachindex(yvec)
                    ...
                    y = yvec[yi]
                    phasei = kxmatg[ki, kj] * x + kymatg[ki, kj] * y - wtmatg[ki, kj] + oi.ϕ
                    phicomp = (ampg_newwave_norm[ki, kj] / (wmatg[ki, kj] + 0.000000001)) * g * ((cosh(kmatg[ki, kj] * (η[xi, yi] + oi.depth)) / cosh(kmatg[ki, kj] * oi.depth)) * sin(phasei))
